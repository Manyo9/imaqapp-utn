...
import { RentalDetailIDDTO, RentalIDDTO } from 'src/app/models/Rental';
...
@Component({
  selector: 'app-details-rentals',
  templateUrl: './details-rentals.component.html',
  styleUrls: ['./details-rentals.component.scss']
})
export class DetailsRentalsComponent {

  request: RequestIDDTO;
  rental: RentalIDDTO;
  details: RentalDetailIDDTO[] = [];
  ...
  private loadRequest(): void {
    this.subs.add(
      this.activatedRoute.params.subscribe((params) => {
        ...
        this.subs.add(
          this.requestService.getRentalRequest(id).subscribe({
            next: (result: ApiResult) => {
              if (result.ok && result.result) {
                this.request = result.result.request as RequestIDDTO;
                this.rental = result.result.rental as RentalIDDTO;
                this.details = result.result.details as RentalDetailIDDTO[];
              } else {
